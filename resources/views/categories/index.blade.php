...
<div class="max-w-7xl mx-auto px-6 py-8">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Categorías') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('Administra las categorías de tus productos') }}</p>
        </div>
    ...
    @if(session('status') === 'category-created')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Categoría creada exitosamente') }}
        </div>
    @endif
    ...
    @if(session('status') === 'category-updated')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Categoría actualizada exitosamente') }}
        </div>
    @endif
    ...
    @if(session('status') === 'category-deleted')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Categoría eliminada exitosamente') }}
        </div>
    @endif
        ...
        <form method="GET" action="{{ route('categorias.index') }}" class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input
                    ...
                    placeholder="{{ __('Buscar categorías...') }}"
            ...
            @if(request('search') || request('status') !== null)
                <a href="{{ route('categorias.index') }}" class="px-5 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-500 hover:border-gray-400 hover:text-gray-700 transition-colors no-underline">{{ __('Limpiar') }}</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @if($categories->count() > 0)
            <table class="w-full text-sm border-collapse">
                <thead class="bg-gray-50 border-b-2 border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Categoría Padre') }}</th>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Descripción') }}</th>
                    ...
                    @foreach($categories as $category)
                        <tr class="hover:bg-gray-50 transition-colors [&:not(:last-child)>td]:border-b [&:not(:last-child)>td]:border-gray-200">
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-gray-100 text-gray-500">{{ __('Raíz') }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('categorias.edit', $category) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors no-underline" title="{{ __('Editar') }}">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 px-4 py-3.5 border-t border-gray-200 text-sm">
                <div class="text-gray-500">
                    {{ __('Mostrando :desde–:hasta de :total categorías', ['desde' => $categories->firstItem(), 'hasta' => $categories->lastItem(), 'total' => $categories->total()]) }}
                </div>
                <div>
                    {{ $categories->links() }}
                </div>
        ...
        @else
            <div class="text-center py-16 px-8 text-gray-500">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ __('No hay categorías') }}</h3>
                <p class="text-sm">{{ __('Comienza creando tu primera categoría de productos.') }}</p>
            </div>
        @endif
    </div>
</div>
