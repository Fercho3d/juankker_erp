...
<div class="max-w-7xl mx-auto px-6 py-8">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Productos') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('Administra tu catálogo de productos') }}</p>
        </div>
    ...
    @if(session('status') === 'product-created')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Producto creado exitosamente') }}
        </div>
    @endif
    ...
    @if(session('status') === 'product-updated')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Producto actualizado exitosamente') }}
        </div>
    @endif
    ...
    @if(session('status') === 'product-deleted')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Producto eliminado exitosamente') }}
        </div>
    @endif
    ...
    <div class="mb-6">
        <form method="GET" action="{{ route('productos.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            <select name="tipo_producto" class="px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white cursor-pointer min-w-[160px] focus:outline-none focus:border-indigo-500">
                ...
                <option value="simple" {{ request('tipo_producto') === 'simple' ? 'selected' : '' }}>{{ __('Simple') }}</option>
                <option value="variable" {{ request('tipo_producto') === 'variable' ? 'selected' : '' }}>{{ __('Variable') }}</option>
            </select>
            ...
            @if(request('search') || request('category_id') || request('brand_id') || request('tipo_producto') || request('status') !== null)
                <a href="{{ route('productos.index') }}" class="px-5 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-500 hover:border-gray-400 hover:text-gray-700 transition-colors no-underline">{{ __('Limpiar') }}</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @if($products->count() > 0)
            <table class="w-full text-sm border-collapse">
                <thead class="bg-gray-50 border-b-2 border-gray-200">
                    <tr>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Código') }}</th>
                        ...
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Categoría') }}</th>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Marca') }}</th>
                    ...
                    @foreach($products as $product)
                        <tr class="hover:bg-gray-50 transition-colors [&:not(:last-child)>td]:border-b [&:not(:last-child)>td]:border-gray-200">
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-gray-100 text-gray-500">{{ __('Sin categoría') }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                @if($product->brand)
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                @if($product->tipo_producto === 'simple')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-emerald-50 text-emerald-800">{{ __('Simple') }}</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-violet-50 text-violet-800">{{ __('Variable') }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                <div class="flex gap-2">
                                    <a href="{{ route('productos.edit', $product) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors" title="{{ __('Editar') }}">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 px-4 py-3.5 border-t border-gray-200 text-sm">
                <div class="text-gray-500">
                    {{ __('Mostrando :desde–:hasta de :total productos', ['desde' => $products->firstItem(), 'hasta' => $products->lastItem(), 'total' => $products->total()]) }}
                </div>
        ...
        @else
            <div class="text-center py-16 px-8 text-gray-500">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ __('No hay productos') }}</h3>
                <p class="text-sm">{{ __('Comienza creando tu primer producto.') }}</p>
            </div>
        @endif
    </div>
</div>
