...
<div class="min-h-[80vh] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <button class="w-full py-2.5 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">{{ __('Crear cuenta gratis') }}</button>
        </form>

        <p class="text-sm text-gray-500 mt-6 text-center">{{ __('¿Ya tienes cuenta?') }}
            <a href="{{ route('login') }}" class="text-blue-600 font-semibold hover:underline">{{ __('Inicia sesión') }}</a>
        </p>
    </div>
</div>
@endsection
