    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-between text-sm">
                <label class="flex items-center gap-2 text-gray-600"><input type="checkbox" name="remember"> {{ __('Mantener sesión') }}</label>
                <a href="{{ route('password.request') }}" class="text-blue-600 hover:underline">{{ __('¿Olvidaste tu contraseña?') }}</a>
            </div>
            @include('partials.turnstile')
            <button class="w-full py-2.5 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">{{ __('Entrar') }}</button>
        </form>

        <p class="text-sm text-gray-500 mt-6 text-center">{{ __('¿No tienes cuenta?') }}
            <a href="{{ route('register') }}" class="text-blue-600 font-semibold hover:underline">{{ __('Prueba 30 días gratis') }}</a>
        </p>
    </div>
</div>
@endsection
