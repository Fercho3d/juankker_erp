...
<div class="min-h-[80vh] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-1">{{ __('Recuperar contraseña') }}</h1>
        <p class="text-sm text-gray-500 mb-6">{{ __('Te enviaremos un enlace para restablecerla.') }}</p>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
            ...
            <button class="w-full py-2.5 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">{{ __('Enviar enlace') }}</button>
        </form>
        <p class="text-sm text-gray-500 mt-6 text-center"><a href="{{ route('login') }}" class="text-blue-600 hover:underline">{{ __('Volver a iniciar sesión') }}</a></p>
    </div>
</div>
@endsection
