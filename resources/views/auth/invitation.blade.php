...
<div class="min-h-[80vh] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        <span class="inline-block px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold mb-3">Invitación · 30 días gratis</span>
        <h1 class="text-2xl font-bold text-gray-900 mb-1">Activa tu cuenta</h1>
        <p class="text-sm text-gray-500 mb-6">Invitación para <strong>{{ $invitation->email }}</strong>.</p>
        ...
        <form method="POST" action="{{ route('invitation.accept', $invitation->token) }}" class="space-y-4">
            ...
            <button class="w-full py-2.5 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">Activar cuenta</button>
        </form>
    </div>
</div>
@endsection
