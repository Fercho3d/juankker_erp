...
<div class="min-h-[80vh] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
        <h1 class="text-2xl font-bold text-gray-900 mb-1">{{ __('Verifica tu correo') }}</h1>
        <p class="text-sm text-gray-500 mb-6">{{ __('Escribe el código de 6 dígitos que te enviamos.') }}</p>
        ...
        <form method="POST" action="{{ route('register.verify') }}" class="space-y-4">
            ...
            <button class="w-full py-2.5 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">{{ __('Verificar y activar') }}</button>
        </form>

        <form method="POST" action="{{ route('register.resend') }}" class="mt-4">@csrf
            <button class="text-sm text-blue-600 hover:underline">{{ __('Reenviar código') }}</button>
        </form>
    </div>
</div>
@endsection
