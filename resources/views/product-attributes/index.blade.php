...
<div class="max-w-7xl mx-auto px-6 py-8">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Atributos de Productos') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('Define los atributos para variantes (Talla, Color, etc.)') }}</p>
        </div>
    ...
    @if(session('status') === 'attribute-created')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Atributo creado exitosamente') }}
        </div>
    @endif
    ...
    @if(session('status') === 'attribute-updated')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Atributo actualizado exitosamente') }}
        </div>
    @endif
    ...
    @if(session('status') === 'attribute-deleted')
        <div class="flex items-center gap-2 px-4 py-3 mb-5 text-sm text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            {{ __('Atributo eliminado exitosamente') }}
        </div>
    @endif
        ...
        <form method="GET" action="{{ route('atributos-producto.index') }}" class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input
                    ...
                    placeholder="{{ __('Buscar atributos...') }}"
            ...
            <select name="status" class="px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white cursor-pointer min-w-[160px] focus:outline-none focus:border-indigo-500">
                ...
                <option value="1" {{ request('status') === '1' ? 'selected' : '' }}>{{ __('Activos') }}</option>
                <option value="0" {{ request('status') === '0' ? 'selected' : '' }}>{{ __('Inactivos') }}</option>
            </select>

            <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-xl hover:bg-indigo-700 transition-colors cursor-pointer">{{ __('Buscar') }}</button>

            @if(request('search') || request('status') !== null)
                <a href="{{ route('atributos-producto.index') }}" class="px-5 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-500 hover:border-gray-400 hover:text-gray-700 transition-colors no-underline">{{ __('Limpiar') }}</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @if($attributes->count() > 0)
            <table class="w-full text-sm border-collapse">
                <thead class="bg-gray-50 border-b-2 border-gray-200">
                    <tr>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Nombre') }}</th>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Tipo') }}</th>
                        <th class="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 whitespace-nowrap">{{ __('Valores') }}</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors [&:not(:last-child)>td]:border-b [&:not(:last-child)>td]:border-gray-200">
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                @if($attribute->tipo === 'select')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-blue-50 text-blue-800">{{ __('Selección') }}</span>
                                @elseif($attribute->tipo === 'color')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-violet-50 text-violet-800">{{ __('Color') }}</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-gray-100 text-gray-500">{{ __('Texto') }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                <a href="{{ route('atributos-producto.valores.index', $attribute) }}" class="text-indigo-600 hover:text-indigo-800 no-underline">
                                    {{ trans_choice(':n valor|:n valores', $attribute->values->count(), ['n' => $attribute->values->count()]) }}
                                </a>
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-gray-900 align-middle">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('atributos-producto.valores.index', $attribute) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors no-underline" title="{{ __('Gestionar valores') }}">
                                        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path d="M3 12h18M3 6h18M3 18h18"/>
                                        </svg>
                                    </a>
                                    <a href="{{ route('atributos-producto.edit', $attribute) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors no-underline" title="{{ __('Editar') }}">
                                        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 px-4 py-3.5 border-t border-gray-200 text-sm">
                <div class="text-gray-500">
                    {{ __('Mostrando :desde–:hasta de :total atributos', ['desde' => $attributes->firstItem(), 'hasta' => $attributes->lastItem(), 'total' => $attributes->total()]) }}
                </div>
                <div>
                    {{ $attributes->links() }}
                </div>
        ...
        @else
            <div class="text-center py-16 px-8 text-gray-500">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ __('No hay atributos') }}</h3>
                <p class="text-sm">{{ __('Crea atributos como Talla, Color o Peso para tus productos variables.') }}</p>
            </div>
        @endif
    </div>
</div>
