...
<div class="max-w-5xl mx-auto px-6 py-8">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-2">{{ __('Papelera') }}</h1>
        <p class="text-sm text-gray-500 mt-0.5">
            {{ __(':n descartados. No cuentan en el embudo ni vuelven a entrar cuando importas del DENUE.', ['n' => number_format($leads->total())]) }}
        </p>
    </div>
    ...
    <form method="GET" class="flex flex-wrap items-center gap-2.5 mb-5">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Buscar empresa, giro, teléfono…') }}"
        ...
        <select name="motivo" onchange="this.form.submit()"
            ...
            <option value="">{{ __('Cualquier motivo') }}</option>
            @foreach (\App\Models\Lead::MOTIVOS_DESCARTE as $valor => $etiqueta)
                <option value="{{ $valor }}" @selected(request('motivo') === $valor)>{{ __($etiqueta) }}</option>
            @endforeach
        </select>
        @if (request('search') || request('motivo'))
            <a href="{{ route('crm.papelera') }}" class="px-3 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-900 no-underline">{{ __('Limpiar') }}</a>
        @endif
    </form>
                ...
                <div class="flex items-center gap-4 px-4 py-3 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                    <div class="min-w-0 flex-1">
                        ...
                        <span class="block text-xs text-gray-500 truncate">
                            {{ collect([$lead->sector ? __($lead->sector) : null, $lead->rangoPersonal() ? __(':rango pers.', ['rango' => $lead->rangoPersonal()]) : null, $lead->municipio, $lead->telefono])->filter()->implode(' · ') }}
                        </span>
                    </div>
                    <span class="shrink-0 hidden sm:inline text-[11px] px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 font-medium">
                        {{ __(\App\Models\Lead::MOTIVOS_DESCARTE[$lead->motivo_descarte] ?? 'Sin motivo') }}
                    </span>
                    ...
                    <form method="POST" action="{{ route('crm.leads.restaurar', $lead->id) }}" class="shrink-0">
                        @csrf
                        <button type="submit" class="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-semibold text-gray-700 hover:border-indigo-400 hover:text-indigo-700 bg-white cursor-pointer">{{ __('Restaurar') }}</button>
                    </form>
                </div>
            @endforeach
        </div>
    ...
    @else
        <div class="bg-white rounded-xl border border-gray-200 px-4 py-10 text-center">
            <p class="text-sm text-gray-500 m-0">
                {{ request('search') || request('motivo') ? __('Nada descartado con esos filtros.') : __('La papelera está vacía. Arrastra al final del embudo a quien no sea prospecto.') }}
            </p>
        </div>
    @endif
</div>
